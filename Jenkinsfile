       ...
       withCredentials(
        [[
            ...
            stage("create cluster configurations"){
                ...
                sh 'kops create cluster k8s.taleas.in --zones eu-central-1b --node-size t2.micro --master-size t2.micro --node-count 2 --master-zones eu-central-1b --ssh-public-key /root/.ssh/id_rsa.pub --state s3://k8s.taleas.in --dns-zone Z1NYSZ17QD2UGG --yes'
