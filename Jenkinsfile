           ...
           stage("Expose environment variable"){
            ...
            stage("create cluster configurations"){
               sh 'sudo chmod -R 777 /root/'
                ...
                sh 'kops create cluster k8s.tea.in --zones eu-central-1b --node-size t2.micro --master-size t2.micro --node-count 2 --master-zones eu-central-1b --ssh-public-key /root/.ssh/id_rsa.pub --state s3://k8s.taleas.in --dns-zone Z3FF4ZWH9AXV3P --yes'
